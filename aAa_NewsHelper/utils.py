...
def imagecheck(teamName,url):
    teamName = teamName.replace(" ","_")
    teamName = teamName.replace("(","")
    teamName = teamName.replace(")","")
    aaa_url = 'https://www.team-aaa.com/upload/admin/Logos/' + teamName +'.png'
    r = requests.get(aaa_url)
    if r.status_code == 404:
        response = requests.get(url)
        with open('Logos/'+ teamName + ".png", 'wb') as f:
            f.write(response.content)
        print("WARNING: UPLOAD LES LOGOS DANS /LOGOS")
        return 'https://www.team-aaa.com/upload/admin/Logos/' + teamName + '.png'
    else:
        return 'https://www.team-aaa.com/upload/admin/Logos/' + teamName + '.png'

def imageTeamName(teamName):
    teamName = teamName.replace(" ","_")
    teamName = teamName.replace("(","")
    teamName = teamName.replace(")","")
...
def AutoStandings(data):
    ...
    #listddata = defaultdict(list)
    #for keys in ddata:
    #    listddata['Standings'] = []
    #    listddata['Standings'].append(keys['losses'],keys['name'],)
...
if __name__ == '__main__':
    ...
    pprint(AutoStandings(Leaguepedia.getGames('LPL 2020 Summer')))
    #imagecheck('Afreeca Freecs','https://gamepedia.cursecdn.com/lolesports_gamepedia_en/7/70/Afreeca_Freecslogo_square.png?version=4c96509aef782666d1eaa06c48a54bf2')